...
public class MyFeatureExtractor {
    ...
    public void setDoubleArr(Double[] a_doubleArr){
        ...
        int i = 0;
        for (Double d : a_doubleArr)
            m_doubleArr[i++]=(double)d;
    ...
    public double getMedianAbsoluteDeviation(){
        ...
        int i=0;
        for (double d : m_doubleArr)
            m_dAbsElementSubMedian[i++]= Math.abs(d - median);

        Arrays.sort(m_dAbsElementSubMedian);

        return m_dAbsElementSubMedian[(int)((m_dAbsElementSubMedian.length-1)/2)];
    }
