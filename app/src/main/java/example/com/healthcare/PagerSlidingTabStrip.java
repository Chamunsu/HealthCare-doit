...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.setBackgroundColor(0xFF242424);
        tabsContainer = new LinearLayout(context);
        tabsContainer.setOrientation(LinearLayout.HORIZONTAL);
        tabsContainer.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
        tabsContainer.setBackgroundColor(0xFF242424);
    ...
    private void addIconTab(final int position, int resId, String title) {
        ...
        //layout.setBackgroundColor(0x00333333);
        tab[position] = new ImageButton(getContext());
        tab[position].setLayoutParams(new ViewGroup.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
        tab[position].setFocusable(true);
        tab[position].setImageResource(resId);
        tab[position].setPadding(0,0,0,0);
        tab[position].setBackgroundColor(0x00242424);
        switch(position)
        {
            case 0:
                tab[position].setImageResource(R.drawable.total_icon_on);
                tab[position].setId(R.id.tab_total);
                break;
            case 1:
                tab[position].setId(R.id.tab_walk);
                break;
            case 2:
                tab[position].setId(R.id.tab_run);
                break;
            case 3:
                tab[position].setId(R.id.tab_pushup);
                break;
            case 4:
                tab[position].setId(R.id.tab_situp);
                break;
        }

        tab[position].setId(R.id.tab_total);

        tab[position].setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                pager.setCurrentItem(position);
            }
        ...
        text.setSingleLine();
        text.setPadding(0,60,0,0);
        text.setTextColor(Color.WHITE);
        text.setTextSize(11);


        layout.addView(tab[position]);
        layout.addView(text,relativeParams);
        ...
        public void onPageSelected(int position) {
            ...
            switch(position)
            {
                case 0:
                    ...
                    break;
                case 1:
                    tab[position].setImageResource(R.drawable.walking_icon_on);
                    break;
                case 2:
                    tab[position].setImageResource(R.drawable.running_icon_on);
                    break;
                case 3:
                    tab[position].setImageResource(R.drawable.pressup_icon_on);
                    break;
                case 4:
                    tab[position].setImageResource(R.drawable.situp_icon_on);
                    break;
            }
            inti_tab_image(position);

        }
    }

    public void inti_tab_image(int position){
        for(int i=0 ;i < tab.length;i++)
        {
            if(position !=i)
                tab[i].setImageResource(((IconTabProvider) pager.getAdapter()).getPageIconResId(i));
        }
    }
