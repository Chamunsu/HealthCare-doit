...
public class CSV_DataUnit {
	...
	public void addAccelerateData(double x, Double y, Double z, int t){
		if(accX.size() == accY.size() && accY.size() == accZ.size() && accZ.size() == accT.size()){
			accX.add(x);
			accY.add(y);
			accZ.add(z);
			accT.add(t);
			...
			System.out.println(accX.size()+",  " + accY.size()+",  " + accZ.size()+",  " + accT.size());
		}
	}
	public void addGyroscopeData(Double x, Double y, Double z, int t){
		if(gyroX.size() == gyroY.size() && gyroY.size() == gyroZ.size() && gyroZ.size() == gyroT.size()){
			gyroX.add(x);
			gyroY.add(y);
			gyroZ.add(z);
			gyroT.add(t);
			...
			System.out.println(gyroX.size()+",  " + gyroY.size()+",  " + gyroZ.size()+",  " + gyroT.size());
		}
	}
	public void printUnits(int type){
		if(accX.size() == accY.size() && accY.size() == accZ.size() && accZ.size() == accT.size()
			&& gyroX.size() == gyroY.size() && gyroY.size() == gyroZ.size() && gyroZ.size() == gyroT.size()){
			int size = type== finalStaticVariables.ACCELERATE_DATA_TYPE? accX.size() : gyroX.size();
			printUnits(type, size);
		}
		else{
			System.out.println("Accelerate or Gyroscope data's size is different please check!");
			System.out.println(accX.size()+",  " + accY.size()+",  " + accZ.size()+",  " + accT.size());
		}
	}
	public void printUnits(int type, int size){
		...
		case finalStaticVariables.ACCELERATE_DATA_TYPE:
			for(int i=0; i<size; i++){
				System.out.print("("+accX.get(i) + ", " + accY.get(i)
						+ ", " + accZ.get(i) + "," + accT.get(i) + ")\t");
			}
			break;
		case finalStaticVariables.GYROSCOPE_DATA_TYPE:
			for(int i=0; i<size; i++){
				System.out.print("("+gyroX.get(i) + ", " + gyroY.get(i)
						+ ", " + gyroZ.get(i) + ", " + gyroT.get(i) + ")\t");
			}
			break;
		default:
