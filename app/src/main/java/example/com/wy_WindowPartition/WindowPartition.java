 ...
 * 클래스 하나가 하는 일이 많다. 수고가 많다.
 */

public class WindowPartition {

	private File[]folderList;
	//folders are divided by name( ex) WY_YK, HJH)
    ...
    String m_filePath = finalStaticVariables.DIR_FOLDER + finalStaticVariables.ACTIVITY + "/";
	...
	public void init(){
		...
		for(File tempFile : folderList) {
			...
			finalStaticVariables.debugPrintln("Path=" + tempPath + "/" + tempFileName);
	...
	public Double[] getCSVdata(int type, int direction, int msWindowArea, int msDuplicatedArea){
		if (mUnits[mUnitPos] != null) {
			...
			case finalStaticVariables.X_DIRECTION:
				if(type == finalStaticVariables.ACCELERATE_DATA_TYPE)
					return (Double[]) mUnits[mUnitPos].getAccX().toArray(new Double[0]); // List ����� �����͸� Array ����� �����ͷ� ��ȯ
				else if(type == finalStaticVariables.GYROSCOPE_DATA_TYPE)
					return (Double[]) mUnits[mUnitPos].getGyroX().toArray(new Double[0]); // List ����� �����͸� Array ����� �����ͷ� ��ȯ
			case finalStaticVariables.Y_DIRECTION:
				if(type == finalStaticVariables.ACCELERATE_DATA_TYPE)
					return (Double[]) mUnits[mUnitPos].getAccY().toArray(new Double[0]); // List ����� �����͸� Array ����� �����ͷ� ��ȯ
				else if(type == finalStaticVariables.GYROSCOPE_DATA_TYPE)
					return (Double[]) mUnits[mUnitPos].getGyroY().toArray(new Double[0]); // List ����� �����͸� Array ����� �����ͷ� ��ȯ
			case finalStaticVariables.Z_DIRECTION:
				if(type == finalStaticVariables.ACCELERATE_DATA_TYPE)
					return (Double[]) mUnits[mUnitPos].getAccZ().toArray(new Double[0]); // List ����� �����͸� Array ����� �����ͷ� ��ȯ
				else if(type == finalStaticVariables.GYROSCOPE_DATA_TYPE)
					return (Double[]) mUnits[mUnitPos].getGyroZ().toArray(new Double[0]); // List ����� �����͸� Array ����� �����ͷ� ��ȯ
			default:
	...
	public void divideCSVdata (int msWindowArea, int msDuplicatedArea) throws IOException {
        ...
        String strFeaturesColumnNames = "Label,"+
            ...
            for(String path : mPaths){
                ...
                while ((nextLine = reader.readNext()) != null) {
                    ...
                    int namePos = path.length() - finalStaticVariables.NAME_POSITION_FROM_BACK;
...
//					System.out.println(path.substring(path.length() - Utils.NAME_POSITION_FROM_BACK, path.length() - Utils.NAME_POSITION_FROM_BACK + Utils.NAME_LENTH));
                    ...
                    finalStaticVariables.debugPrintln(nextLine[0] + "\t|\t" + nextLine[1]
                            + "\t|\t" + nextLine[2] + "\t|\t" + nextLine[3] + "\n");

                    if(path.substring(namePos, namePos+ finalStaticVariables.NAME_LENTH).equals(finalStaticVariables.NAME_ACCE_9)){
                        isAcc = true;
                        mUnits[mUnitPos].addAccelerateData(x,y,z,t);
                        if(finalStaticVariables.DEBUG){
                            System.out.print("new["+mUnitPos+"] one: \t");
                            mUnits[mUnitPos].printUnits(finalStaticVariables.ACCELERATE_DATA_TYPE);
                        }
                        if(isOldUnitActive){
                            mUnits[nUnitPos].addAccelerateData(x,y,z,t);
                            if(finalStaticVariables.DEBUG){
                                System.out.print("old["+nUnitPos+"] one: \t");
                                mUnits[nUnitPos].printUnits(finalStaticVariables.ACCELERATE_DATA_TYPE);
                            }
                        }
                    }
                    else if(path.substring(namePos, namePos+ finalStaticVariables.NAME_LENTH).equals(finalStaticVariables.NAME_GYRO_9)){
                        isAcc = false;
                        mUnits[mUnitPos].addGyroscopeData(x,y,z,t);
                        if(finalStaticVariables.DEBUG) {
                            System.out.print("new[" + mUnitPos + "] one: \t");
                            mUnits[mUnitPos].printUnits(finalStaticVariables.GYROSCOPE_DATA_TYPE);
                        }
                        if(isOldUnitActive){
                            mUnits[nUnitPos].addGyroscopeData(x,y,z,t);
                            if(finalStaticVariables.DEBUG){
                                System.out.print("old["+nUnitPos+"] one: \t");
                                mUnits[nUnitPos].printUnits(finalStaticVariables.GYROSCOPE_DATA_TYPE);
                            }
                        }
                    }
                    else{
                        System.out.println("Can't know :"+path.substring(namePos, namePos+ finalStaticVariables.NAME_LENTH));
                    }
                    ...
                    if(t/(msWindowArea - msDuplicatedArea) > flagNew){
                        ...
                        finalStaticVariables.debugPrintln("new one( mUnits[" + mUnitPos + "] begin.  " + t / (msWindowArea - msDuplicatedArea) + ">" + flagNew);
                        flagNew = t/(msWindowArea - msDuplicatedArea);
                    }
                    else if (flag && t > flagNew * (msWindowArea - msDuplicatedArea) + 500) {
                        finalStaticVariables.debugPrintln("old one( mUnits[" + nUnitPos + "] end.");
                        flag = false;
                        isOldUnitActive = false;
                        //End in here.
                        //JH Code START
                        ...
                        if (isAcc) {
                            doubleArrayX = (Double[]) mUnits[nUnitPos].getAccX().toArray(new Double[0]);
                            doubleArrayY = (Double[]) mUnits[nUnitPos].getAccY().toArray(new Double[0]);
                            doubleArrayZ = (Double[]) mUnits[nUnitPos].getAccZ().toArray(new Double[0]);

                            mfe.setDoubleArr(doubleArrayX); //MyFeatureExtractor 객체에 데이터 Array 전달
                            double[] features = mfe.getFeatures();
                            AccBW.write(m_dLabel+","+features[0]+","+features[1]+","+features[2]+","+features[3]+","+features[4]+","+features[5]+","+features[6]+","+features[7]+",");

                            mfe.setDoubleArr(doubleArrayY);
                            features = mfe.getFeatures();
                            AccBW.write(features[0]+","+features[1]+","+features[2]+","+features[3]+","+features[4]+","+features[5]+","+features[6]+","+features[7]+",");
                            ...
                            for (int i = 0; i <  arrLength; i++)
                                doubleArrayXSubY[i] = doubleArrayX[i] - doubleArrayY[i]; //X-Y array

                            Double[] doubleArrayYSubZ = new Double[arrLength];

                            for (int i = 0; i <  arrLength; i++)
                                doubleArrayYSubZ[i] = doubleArrayY[i] - doubleArrayZ[i]; //Y-Z array

                            Double[] doubleArrayZSubX = new Double[arrLength];

                            for (int i = 0; i <  arrLength; i++)
                                doubleArrayZSubX[i] = doubleArrayZ[i] - doubleArrayX[i]; //Z-X array
                            ...
                            doubleArrayX = (Double[]) mUnits[nUnitPos].getGyroX().toArray(new Double[0]);
                            doubleArrayY = (Double[]) mUnits[nUnitPos].getGyroY().toArray(new Double[0]);
                            doubleArrayZ = (Double[]) mUnits[nUnitPos].getGyroZ().toArray(new Double[0]);

                            mfe.setDoubleArr(doubleArrayX); //MyFeatureExtractor 객체에 데이터 Array 전달
                            double[] features = mfe.getFeatures();
                            GyroBW.write(m_dLabel+","+features[0]+","+features[1]+","+features[2]+","+features[3]+","+features[4]+","+features[5]+","+features[6]+","+features[7]+",");

                            mfe.setDoubleArr(doubleArrayY);
                            features = mfe.getFeatures();
                            GyroBW.write(features[0]+","+features[1]+","+features[2]+","+features[3]+","+features[4]+","+features[5]+","+features[6]+","+features[7]+",");
                            ...
                            for (int i = 0; i < doubleArrayX.length; i++)
                                doubleArrayXSubY[i] = doubleArrayX[i] - doubleArrayY[i]; //X-Y array
